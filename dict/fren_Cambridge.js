...
class fren_Cambridge {
    ...
    async displayName() {
        ...
        if (locale.indexOf('EN') != -1) return 'French -> English | Cambridge';
        return 'Francés -> Inglés | Cambridge';
    }
            ...
            for (const defblock of defblocks) {
                ...
                if (examps.length > 0 && this.maxexample > 0) {
                    ...
                    for (const [index, examp] of examps.entries()) {
                        ...
                        definition += `<li class='sent'><span class='eng_sent'>${eng_examp}</span></br><span class='chn_sent'>${chn_examp}</span></li>`;
                    }
                    definition += '</ul>';
                }
